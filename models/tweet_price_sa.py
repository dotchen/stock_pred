...
from .attention import SelfAttention, CrossAttention, pos_embd

class TweetPriceSAModel(BasicModel):
    def __init__(self, embedding_matrix, config):
        ...
        self.tweet_sa   = SelfAttention(self.tweet_hidden_dim+self.price_hidden_dim, 32, self.tweet_hidden_dim+self.price_hidden_dim)
        ...
        self.predict = nn.Sequential(
            ...
            nn.Linear(self.tweet_hidden_dim+self.price_hidden_dim, len(config['price_shifts_bins'])+1)
        )
    ...
    def forward(self, price_hist, price_lens, tweet_hist, tweet_lens):
        ...
        queries = price_embd.view(-1,1,self.price_hidden_dim)
        # Add position embedding
        keys = tweet_embd.permute(0,2,3,1,4).view(-1,T_tw,self.tweet_hidden_dim) + pos_embd(T_tw, self.tweet_hidden_dim, device=tweet_embd.device)
        keys = tweet_embd.view(B*C,-1,self.tweet_hidden_dim)

        embd = self.tweet_attn(
            queries, keys,
            mask=(tweet_lens>0).permute(0,2,3,1).contiguous().view(B*C,N_tw*T_tw)
        ).view(B,C,-1)

        # import pdb; pdb.set_trace()
        # values = tweet_embd.transpose(1,2)
        ...
        # embd = self.bilinear(tweet_embd.mean(dim=1), price_embd)
        embd = self.tweet_sa(torch.cat([embd, price_embd], dim=-1))
        # embd = self.fuse(tweet_embd.mean(dim=1), price_embd)

        return self.predict(embd).permute(0,2,1)
