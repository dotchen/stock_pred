...
class TweetVanillaModel(BasicModel):
    def __init__(self, embedding_matrix, config):
        ...
        self.predict = nn.Linear(self.tweet_hidden_dim + self.price_hidden_dim, len(config['price_shifts_bins'])+1)
        # self.predict = nn.Linear(self.embedding_dim+self.price_hidden_dim, len(config['price_shifts_bins'])+1)
    ...
    def forward(self, price_hist, price_lens, tweet_hist, tweet_lens):
        ...
        mean_tweet_embd, _ = mean_tweet_embd.max(dim=2)                              # Average over tweets
